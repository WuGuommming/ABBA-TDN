...
def back_normalize(tensor, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    
    is_np = not torch.is_tensor(tensor)
    if is_np:
        tensor = torch.from_numpy(tensor)
    orisize = tensor.size()
    newt = tensor.view(-1, 224, 224)
    rep_mean = mean * (newt.size()[0]//len(mean))
    rep_std = std * (newt.size()[0]//len(std))
    for t, m, s in zip(newt, rep_mean, rep_std):
        t.mul_(s).add_(m)
    
    if is_np:
        return newt.view(orisize).numpy()
    return newt.view(orisize)


def for_normalize(tensor, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    is_np = not torch.is_tensor(tensor)
    if is_np:
        tensor = torch.from_numpy(tensor)
    orisize = tensor.size()
    newt = tensor.view(-1, 224, 224)
    for i in range(0, len(newt), 3):
        for j in range(3):
            newt[i+j] = (newt[i+j] - mean[j]) / std[j]
    if is_np:
        return newt.view(orisize).numpy()
    return newt.view(orisize)
