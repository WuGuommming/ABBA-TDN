...
class TSNDataSet(data.Dataset):
    def __init__(self, dataset, root_path, list_file,
                 ...
                 addition_length=0, load_from_att=False):
        ...
        self.addition_length = addition_length  # additional frames for motion blur, motion len=10 new_length=5, total 15 frames for each segment
        if self.dense_sample:
            print('=> Using dense sample for the dataset...')

        self._parse_list()
        self.load_from_att = load_from_att

    def _load_image(self, directory, idx):
        if self.modality == 'RGB':
            try:
                if self.load_from_att:
                    return [Image.open(os.path.join(self.root_path, directory, str(idx)+".jpg")).convert('RGB')]
                else:
    ...
    def _sample_indices(self, video_list):
        if not self.I3D_sample:  # TSN uniformly sampling for TDN
            ...
            if average_duration > 0:
                offsets += list(
                    np.multiply(list(range(self.num_segments)), average_duration) + randint(average_duration,
            ...
            if self.dataset == 'hmdb51':
                ...
                return offsets + 1
        elif self.dataset == 'kinetics' and self.I3D_sample and (
        not self.dense_sample):  # i3d type sampling for training
    ...
    def __getitem__(self, index):
        ...
        if ('something' in self.dataset) or ('hmdb51' in self.dataset) or ('ucf101' in self.dataset):
            decode_boo = False
        ...
        if not self.test_mode:
            if self.I3D_sample:
                ...
                segment_indices = self._sample_indices(video_list) if self.random_shift else self._get_val_indices(
                    video_list)
        else:
    ...
    def get(self, record, video_list, indices, decode_boo):
        ...
        for seg_ind in indices:
            p = int(seg_ind)
            for i in range(p - self.addition_length, p):
                if i <= 0:
                    if self.dataset == "hmdb51":
                        seg_imgs = self._load_image(record.path, 0)
                    else:
                        seg_imgs = self._load_image(record.path, 1)
                else:
                    seg_imgs = self._load_image(record.path, i)
