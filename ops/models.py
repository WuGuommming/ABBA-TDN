...
class TSN(nn.Module):
    ...
    def get_base_out(self, input):
        sample_len = (3 if self.modality == "RGB" else 2) * self.new_length
        input = input.view((-1, sample_len * 5) + input.size()[-2:])
    ...
    def forward_one(self, x):
        ...
        return np.squeeze(self.forward(torch.from_numpy(x[np.newaxis]).cuda()).detach().cpu().numpy(), axis=0)
    ...
    def forward_and_gradient_one(self, x, label):
        ...
        predictions = self.get_base_out(inputs)
        ...
        assert predictions.shape == (self.num_class, )
    ...
    def gradient(self, inputs, labels):
        ...
        target = torch.from_numpy(labels).long().cuda()
        inputs = torch.from_numpy(inputs).cuda()
        inputs.requires_grad_()
    ...
    def _loss_fn(self, x, label):
        ...
        target = np.array([label])
        target = torch.from_numpy(target).long().cuda()
        inputs = torch.from_numpy(x[None]).cuda()
